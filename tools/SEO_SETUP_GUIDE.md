...
이 가이드는 88서울마사지 웹사이트의 SEO 자동화를 설정하는 방법을 설명합니다.
...
2. 웹사이트 등록 (`https://patriciarepiciphysicaltherapy.com`)
...
- 위치: `public/7a8d3c91-2f4e-4b7a-9d2e-1f3a5c8e7d2b.txt`
- 현재 키: `7a8d3c91-2f4e-4b7a-9d2e-1f3a5c8e7d2b`

**⚠️ 실제 키로 변경해야 합니다:**

```bash
# public/7a8d3c91-2f4e-4b7a-9d2e-1f3a5c8e7d2b.txt 수정
# 새로운 키로 교체
echo "your-actual-indexnow-key" > public/7a8d3c91-2f4e-4b7a-9d2e-1f3a5c8e7d2b.txt
```
...
python tools/indexnow.py \
  --domain patriciarepiciphysicaltherapy.com \
  --key 7a8d3c91-2f4e-4b7a-9d2e-1f3a5c8e7d2b \
  --sitemap https://patriciarepiciphysicaltherapy.com/sitemap.xml
```
...
# crontab -e 실행
# 매일 자정에 실행
0 0 * * * cd /path/to/repo && python tools/indexnow.py --config .env.local --sitemap https://patriciarepiciphysicaltherapy.com/sitemap.xml >> /var/log/indexnow.log 2>&1

# 매일 오전 1시에 Google Indexing API 실행
0 1 * * * cd /path/to/repo && python tools/google_indexing_api.py --credentials credentials/google-indexing-sa.json --sitemap https://patriciarepiciphysicaltherapy.com/sitemap.xml >> /var/log/google-indexing.log 2>&1
```
...
- **Location**: `https://patriciarepiciphysicaltherapy.com/sitemap.xml`
...
**해결책**:
1. IndexNow 키 확인 (public/7a8d3c91-2f4e-4b7a-9d2e-1f3a5c8e7d2b.txt와 일치)
2. 도메인 확인 (www 포함/미포함 일관성)
3. 키 파일이 접근 가능한지 확인: `curl https://patriciarepiciphysicaltherapy.com/7a8d3c91-2f4e-4b7a-9d2e-1f3a5c8e7d2b.txt`
...
# Sitemap 생성 확인
curl https://patriciarepiciphysicaltherapy.com/sitemap.xml
```
