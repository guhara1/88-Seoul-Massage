...
2. 웹사이트 등록 (`https://88-seoul-massage.netlify.app`)
...
python tools/indexnow.py \
  --domain 88-seoul-massage.netlify.app \
  --key 7a8d3c91-2f4e-4b7a-9d2e-1f3a5c8e7d2b \
  --sitemap https://88-seoul-massage.netlify.app/sitemap.xml
```
...
# crontab -e 실행
# 매일 자정에 실행
0 0 * * * cd /path/to/repo && python tools/indexnow.py --config .env.local --sitemap https://88-seoul-massage.netlify.app/sitemap.xml >> /var/log/indexnow.log 2>&1

# 매일 오전 1시에 Google Indexing API 실행
0 1 * * * cd /path/to/repo && python tools/google_indexing_api.py --credentials credentials/google-indexing-sa.json --sitemap https://88-seoul-massage.netlify.app/sitemap.xml >> /var/log/google-indexing.log 2>&1
```
...
- **Location**: `https://88-seoul-massage.netlify.app/sitemap.xml`
...
3. 키 파일이 접근 가능한지 확인: `curl https://88-seoul-massage.netlify.app/7a8d3c91-2f4e-4b7a-9d2e-1f3a5c8e7d2b.txt`
...
# Sitemap 생성 확인
curl https://88-seoul-massage.netlify.app/sitemap.xml
```
